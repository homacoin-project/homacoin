...
{
    QString ret = QString("homacoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HOMA unit input in lnReqAmount, we generate the URI with HOMA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(HomacoinUnits::format(HomacoinUnits::HOMA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
