...
uint256 hashGenesisBlock("0xee8d13dede74998d0f6718d3b715a8ebb8da70b7aca151167ac8fa33f645d2a0");
...
const string strMessageMagic = "Homacoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uHOMA
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1) {
      nSubsidy = 3150000 * COIN;
    }
    // Subsidy is cut in half every 210000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 102000);
...
void ThreadScriptCheck() {
    RenameThread("homacoin-scriptch");
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4d17c98cd384b14caef8cb903e48f86c6053e943ef695c8265bf1e9d88a56d0f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The tinestamp / HomaCoin Start";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1504007045;
        block.nBits    = 0x1d00ffff;
        block.nNonce   = 95768;

        if (fTestNet)
        {
            block.nTime    = 1504007046;
            block.nNonce   = 49568;
        }
...
unsigned char pchMessageStart[4] = { 0xd2, 0xb8, 0xf1, 0xe8 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HomacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HomacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HomacoinMiner(CWallet *pwallet)
{
    printf("HomacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("homacoin-miner");
    ...
    try { loop {
        ...
        printf("Running HomacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HomacoinMiner terminated\n");
        throw;
    }
}

void GenerateHomacoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HomacoinMiner, pwallet));
}
